/*
 * @Author: freeze.xue 
 * @Date: 2020-10-27 14:38:16 
 * @Last Modified by: freeze.xue
 * @Last Modified time: 2020-10-27 14:39:08
 */
"use strict";
...
class UserService extends Service {
  ...
  async register(params) {
    const { ctx, app } = this;
    let { username, password, avatar, authCode } = params; //接受客户端的参数
    ...
    if (authCode !== ctx.session.code) {
      ...
      //console.log("UserService -> register -> result2", result2)
      ...
      //插入用户角色
      await app.mysql.query(insertRole, [1, result3[0].user_id]);
      //之前的版本 外键约束存在问题 需要根本表结构在做调整  目前调整的是对的
      if (result2) {
        result = {
          code: 200,
          message: "注册成功",
  ...
  async login(params) {
    ...
    // https://blog.csdn.net/BaiXuePrincess/article/details/100163498 此处 字符串类型的必须加引号给包括起来 下边的是一个id数字 这个就无所谓了
    ...
    if (result1.length > 0) {
      //比较加密后密码
      await ctx.helper
        .comparePassword(password, result1[0].password)
        .then((isMatch) => {
          if (isMatch) {
            //ctx.helper.getToken
            // const token = app.jwt.sign(
              const token = ctx.helper.getToken(
              { id: result1[0].user_id, name: result1[0].username},
              secret,
              { expiresIn: '60m' }, 
              );
            
            //console.log('cxt.state.user=========',cxt.state.user)
      ...
      .then(async (res) => {
        if (res) {
          ...
          if (result1[0]) {
            ...
            let sqlStr =
              'UPDATE cart SET product_amount = "' +
              product_amount +
              '" WHERE  product_id = "' +
              product_id +
              '"';
            ...
            //如果商品不存在，则插入商品到数据库
            let add_str =
              "INSERT INTO cart(user_id,product_id,product_name,price,product_amount,product_image) VALUES(?,?,?,?,?,?)";
            let sql_params = [
              user_id,
              product_id,
              product_name,
